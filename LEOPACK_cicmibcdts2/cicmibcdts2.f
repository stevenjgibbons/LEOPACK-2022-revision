...
C Steve Gibbons - Inhomogeneous Boundary Conv. + Dynamo Time Step    C
...
C
     ...
     3                 NIV3MX, NIV4MX, NIV5MX, NCMXX,
     4                 NIVCMX, NHCMAX, NRMAXC, NDCSV, NDCSM
      PARAMETER      ( NRVMAX = 80, NRMMAX = 120, NH1MAX = 200,
     ...
     4                 NIV5MX = NH5MAX*NRMMAX )
      PARAMETER      ( NDCSV = 3, NDCSM = 2+LHMAX, NBNM = 3,
     1                 NCFM = 2*NBNM+1, NDRVM = 4, NNDM = 4 )
      PARAMETER      ( NRMAXC = 100,
...
C
     ...
     1                  NCUDSV, NCUDSM, INARRV( 3 ), INARRM( 3 ),
     2                  NHV, NHM, NR1, NDRVS, ILN, IRN, ICOMP, IDIR,
     3                  MMAX, IOPT, INARRC( 3 ), NRC, NHC, NTSBSE,
     3                  NTSBLE, NTSBME, LUNRG, NRCM
      INTEGER           ITS, NTS, NTSBB, NTSBS, N, INC, NDIG, N1, N2,
     1                  K, L, M, LMINM, MMVAL, LLVAL, LUMNRG
      DOUBLE PRECISION  PVLC( NBNM ), PVRC( NBNM ), DMONE, DPONE,
     1                  STIME, DTIME, FAC, SCAL, DTOL2, DKE( 2 ),
     2                  DTOTKE, DEATOT, DTORKE, DLOW,
     3                  DTOTME, DEATOM, DTORME
      CHARACTER *(120)  LINE, FNLOG, ROOT, FNAME, FNNRG, FNMNRG,
     1                  FNAMEV, FNAMEM
      CHARACTER *(10)   CHNUM
C_____________________________________________________________________
C                                                                    C
...
C
      NCUDSV = 0
C     (ncudsv - number of diff. schemes already in use for vel.).
      CALL HMFRD( NHV, NHVMAX, MHTV, MHLV, MHMV, MHPV, NCUDSV, NDCSV,
     1            MHIBCV, MHOBCV, LARRV, LU, FNAME )
      INARRV( 3 ) = NHV
      WRITE ( LULOG, * ) 'PROGRAM cicibcdts2.'
      WRITE ( LULOG, * ) 
...
C
      DTOL2 = 1.0d-5
      CALL ITVAAF( NH3, NRV, ML3, MM3, INARRC, MHTC, MHLC, MHMC,
     1             NNDM, IWRK, XARRV, XARRC, SVC, CD, SCAL, DTOL2,
...
C
      CALL IBCDT1( XARRV,ML1,MM1,IP1,ML2,MM2,IP2,ML3,MM3,IP3,ML4,MM4,
     ...
     6  CT4P,CQ5T,CS5T,DV3C,SV3C,DSV3C,FTFPV,FTFTV,INFPV,INFTV,
...
C         .
        ENDDO
C       .
C       . OK: The array DKEARV now contains the kinetic
C       . energy for each harmonic.
C       .
        DO I = 1, NHM
C         .
...
C       .
     ...
     1               NH5, ML5, MM5, SVM, SV5, IDIR )
C       .
C       . SVV and SVM now contains the new solution vectors,
...
C                                                                    C
C     DV3C      : Dim ( NH3*NRV ). Diffusion forcing from inhomog.   C
C     SV3C      : Dim ( NH3*NRV ). Inhomog. temp. zero^th deriv.     C
C     DSV3C     : Dim ( NH3*NRV ). Inhomog. temp. first deriv.       C
C                                                                    C
C*********************************************************************
      SUBROUTINE IBCDT1(XARRV,ML1,MM1,IP1,ML2,MM2,IP2,ML3,MM3,IP3,ML4,
     ...
     6 VS5,CT4P,CQ5T,CS5T,DV3C,SV3C,DSV3C,FTFPV,FTFTV,INFPV,INFTV,
     7 FTFPM,FTFTM,INFPM,INFTM,FTFPCV,FTFTCV,INFPCV,INFTCV,
     8 FTFPCM,FTFTCM,INFPCM,INFTCM,VGFA,IVGFA,SF2SA,ISF2S,
     9 PFAV,TFAV,JPFAV,JTFAV,PFAM,TFAM,JPFAM,JTFAM)
      IMPLICIT NONE
...
C
      DOUBLE PRECISION DV3C( NH3*NRV ), SV3C( NH3*NRV ),
     1                 DSV3C( NH3*NRV )
C
C____________________________________________________________________C
...
C
      ...
      ENDIF
C
      IF ( LW ) WRITE ( IOUTF, * ) 'Entered IBCDT1.'
C
...
C
     ...
     1             BETA, D1, INCY )
      IF ( LW ) WRITE ( IOUTF, * ) 
     1    'IBCDT1: Called DGBMV: dv1:= bm1 . sv1.'
C
C Toroidal velocity:
...
C
     ...
     1             BETA, D2, INCY )
      IF ( LW ) WRITE ( IOUTF, * ) 
     1    'IBCDT1: Called DGBMV: dv2:= bm2 . sv2.'
C
C Temperature:
...
C
     ...
     1             BETA, D3, INCY )
      IF ( LW ) WRITE ( IOUTF, * ) 
     1    'IBCDT1: Called DGBMV: dv3:= bm3 . sv3.'
C
C Now add on the term from the inhomogeneous part.
C
      CALL DAXPY( N, DELTAT, DV3C, INCX, D3, INCX )
      IF ( LW ) WRITE ( IOUTF, * )
...
C
     ...
     1             BETA, D4, INCY )
      IF ( LW ) WRITE ( IOUTF, * )
     1    'IBCDT1: Called DGBMV: dv4:= bm4 . sv4.'
C
...
C
     ...
     1             BETA, D5, INCY )
      IF ( LW ) WRITE ( IOUTF, * )
     1    'IBCDT1: Called DGBMV: dv5:= bm5 . sv5.'
C
...
C
     ...
     7    VGFA,IVGFA,SF2SA,ISF2S,SV3C,DSV3C,
...
C
      IF ( LW ) WRITE ( IOUTF, * )
     1    'IBCDT1: Iteration ',NOIT,' norm = ', AVEC1
C
      DIFF = DABS( AVEC1 - AOLD )
      IF ( DIFF.LT.DTOL ) THEN
        IF ( LW ) WRITE ( IOUTF, * )
     1    'IBCDT1: Soln. converged iteration ', NOIT
          RETURN
      ENDIF
C
...
C
      IF ( DIFF.GT.ODIFF .AND. NOIT.GT.3 ) THEN
        IF ( LW ) WRITE ( IOUTF, * )
     1    'IBCDT1: Soln. norm increasing: iteration ', NOIT
        NOIT = -2
        RETURN
      ENDIF
C
...
C Inhomogeneous temperature terms
C
      DOUBLE PRECISION SV3C( NH3*NRV ), DSV3C( NH3*NRV )
C
C____________________________________________________________________C
C Variable declarations - Working variables .........................C
...
C*********************************************************************
C subroutine Radial Spectrally Decomposed Vector 2 xtra special F ****
C            -      -          -          -      -              - ****
C Steve Gibbons Thu Dec  6 12:12:25 WET 2001                         C
C____________________________________________________________________C
C                                                                    C
C The double precision arrays QRF, SRF and TRF define sets of        C
C radial functions which constitute a vector with the following      C
...
C          TRF( ( IR  - 1 )*NTH + iht )   for IFORMF = 3             C
C                                                                    C
C Before RSDV2F is called, the arrays FTFP, FTFT, INFP and INFT      C
C must first be calculated by a call to RSDV2C.                      C
C                                                                    C
C____________________________________________________________________C
C                                                                    C
...
C     NRM       : Number of radial grid nodes for magnetic field.    C
C     NRIC      : Number of radial grid nodes in inner core.         C
C     ILNR      : Lowest radial node to be acted upon.               C
C     IRNR      : Highest radial node to be acted upon.              C
C     NPH       : Number of poloidal harmonics [number of radial     C
C                  functions Q(r) and S(r)]                          C
...
C
C*********************************************************************
      SUBROUTINE RSDV2F( NTHP, NPHP, NRV, NRM, NRIC, ILNR, IRNR, NPH,
     1                   NTH, NCMX, ICMR, ICMT, ICMP,
     2                   QRF, SRF, TRF, XSV, FTF1, FTF2, FTF3,
     3                   FTFP, FTFT, INFP, INFT, IFORMF )
      IMPLICIT NONE
C____________________________________________________________________C
C Variable declarations - Parameters ................................C
      INTEGER          NTHP, NPHP, NRV, NRM, NRIC, ILNR, IRNR, NPH,
     1                 NTH, IFORMF, NCMX, ICMR, ICMT, ICMP
      INTEGER          INFP( 2, NPH ), INFT( 2, NTH )
      DOUBLE PRECISION QRF( * ), SRF( * ), TRF( * ),
...
C   
     ...
     2     ICMR.GT.NCMX .OR. ICMT.GT.NCMX .OR. ICMP.GT.NCMX  ) THEN
      ...
      ENDIF
C   
C    NRIC is the number of grid nodes which are strictly in
C    the inner core.
C   
      LENV = 2*NPHP
C     .................. start loop around radial grid nodes
...
C*********************************************************************
C*********************************************************************
C subroutine Xtra Special Vector function 2 Spectrally Decomp. vec F *
C            -    -       -                 -          -           - *
C Steve Gibbons Thu Dec  6 12:44:17 WET 2001                         C
C____________________________________________________________________C
C                                                                    C
C The double precision array XSV contains a vector function of       C
C dimensions ( NCNX, NPHP, NTHP, NRV ). XSVSDF transforms this back  C
C into a spectral space in the following format.                     C
C                                                                    C
C The radial component of the vector is stored in                    C
...
C
C*********************************************************************
      SUBROUTINE XSVSDF( NTHP, NPHP, NRV, NRM, NRIC, ILNR, IRNR, NPH,
     1                   NTH, NCMX, ICMR, ICMT, ICMP,
     2                   QRF, SRF, TRF, XSV, FTF1, FTF2, FTF3,
     3                   PFA, TFA, JPFA, JTFA, IFORMF )
      IMPLICIT NONE
