...
C -    -        -                                                    C
C Thu Dec  6 15:45:49 WET 2001                                       C
C                                                                    C
...
C
     ...
     4        NHV, NHM, NR, IRAD, NRMNW, NRCM, ISPCM
...
C
      ...
      ROM = XARRMN( NRM )
C
C Make sure that the magnetic field goes atleast as
C far as the core mantle boundary
C
      IF ( ROV.GT.ROM ) THEN
...
C
      ...
      ENDIF
C
C Enter information about conducting mantle
C
      PRINT *,' Enter RCM, NRCM, ISPCM '
      PRINT *,' RCM = radius of the outer edge of cond. mantle'
      PRINT *,' NRCM = number of grid points in cond. man.'
      PRINT *,' ISPCM = 1 --> these are evenly spaced nodes.'
      PRINT *,' ISPCM = 2 --> Chebyshev nodes over (-1,1).'
      PRINT *,' ISPCM = 3 --> Chebyshev nodes over (-1,0).'
C
 52   CONTINUE
      READ ( 5, 80 ) LINE
      IF ( LINE(1:1).EQ.'*' ) GOTO 52
      READ ( LINE, * ) RCM, NRCM, ISPCM
C
...
C
      IF ( RCM.LT.ROV ) THEN
        PRINT *,' RCM = ', RCM,' ROV = ', ROV
        PRINT *,' Program aborted.'
        STOP
      ENDIF
C
      IF ( ISPCM.NE.1 .AND. ISPCM.NE.2 .AND. ISPCM.NE.3 ) THEN
        PRINT *,' ISPCM = ', ISPCM
        PRINT *,' Program aborted.'
        STOP
      ENDIF
C
      NRMNW = NRICNW+NRVNW+NRCM
...
C
      ...
      INARMO( 3 ) = NHMUT
C
C Fill radial grid node array for mantle:
C first put the numbers into the temporary array
C XARRM2, and then fill them in reverse order
C
      IF ( NRCM.GT.0 ) THEN
        NR = NRCM + 1
        IF ( ISPCM.EQ.1 ) CALL ESNAAS( NR, XARRM2, ROV, RCM )
        IF ( ISPCM.EQ.2 ) CALL ZCPAAS( NR, XARRM2, ROV, RCM )
        IF ( ISPCM.EQ.3 ) CALL ZCPAA2( NR, XARRM2, ROV, RCM )
      ENDIF
C
      DO IR = 1, NR
        I = NRMNW + 1 - IR
        XARRMT( I ) = RCM + ROV - XARRM2( IR )
      ENDDO
C
...
C
     ...
     1                     WORK1, IWORK, WORK2, WORKM )
              ...
              ELSE
C
C OK so we need to do an extrapolation ...
C first and foremost this means that we should keep
C the gradient constant from the outer boundary
C
                CALL SVRINT( ROM, VECMIN, XARRMN, INARMI, IHI, NNDS,
     1                     WORK1, IWORK, WORK2, WORKM )
                DY = WORK1( 1 )
                DM = WORK1( 2 )
                DC = DY - DM*ROM
                IND = INDFUN( IR, IHO, INARMO )
                VECMUT( IND ) = RAD*DM + DC
C
              ENDIF
            ENDDO
          ENDIF
