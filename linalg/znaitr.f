...
c
c  RNORM   Complex*16 scalar.  (INPUT/OUTPUT)
...
c 
c\SCCS Information: @(#)
c FILE: naitr.F   SID: 2.1   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
...
c
     ...
     &           temp1, rnorm1, wnorm
      Double precision            
     &           ovfl, smlnum, tst1, ulp, unfl, betajr, betaji
      save       first, orth1, orth2, rstart, step3, step4,
     &           ierr, ipj, irj, ivj, iter, itry, j, msglvl, ovfl,
     &           betajr, betaji, rnorm1, smlnum, ulp, unfl, wnorm
c
c     %----------------------%
c     | External Subroutines |
c     %----------------------%
c
      external   zaxpy, zcopy, zscal, zgemv, zgetv0, dlabad, 
     &           zvout, zmout, ivout, second
c
c     %--------------------%
...
c
         ...
         if (dlapy2(dble(rnorm),dimag(rnorm)) .ge. unfl) then
             temp1 = one / rnorm
             call zscal (n, temp1, v(1,j), 1)
             call zscal (n, temp1, workd(ipj), 1)
         else
c
c            %-----------------------------------------%
...
c            %-----------------------------------------%
c
             call zlascl ('General', i, i, dble(rnorm), dble(one),
     &                    n, 1, v(1,j), n, infol)
             call zlascl ('General', i, i, dble(rnorm), dble(one),  
...
c
         if (bmat .eq. 'G') then  
             wnorm = zdotc (n, resid, 1, workd(ipj), 1)
             wnorm = sqrt(dlapy2(dble(wnorm),dimag(wnorm)))
         else if (bmat .eq. 'I') then
...
c
     ...
     &               one, resid, 1)
c
         if (j .gt. 1) h(j,j-1) = dcmplx(betajr, betaji)
c
         call second (t4)
c 
         orth1 = .true.
...
c
         if (bmat .eq. 'G') then         
            rnorm = zdotc (n, resid, 1, workd(ipj), 1)
            rnorm = sqrt(dlapy2(dble(rnorm),dimag(rnorm)) )
         else if (bmat .eq. 'I') then
...
c        %-----------------------------------------------------------%
c
         if (abs(rnorm) .gt. 0.717*abs(wnorm)) go to 100
         iter  = 0
         nrorth = nrorth + 1
c 
c        %---------------------------------------------------%
...
c
         if (msglvl .gt. 2) then
            call zvout (logfil, 1, wnorm, ndigit, 
     &                  '_naitr: re-orthogonalization ; wnorm is')
            call zvout (logfil, 1, rnorm, ndigit, 
     &                  '_naitr: re-orthogonalization ; rnorm is')
         end if
         if (msglvl .gt. 0 .and. iter .gt. 0) then
            call zvout (logfil, 1, rnorm1, ndigit, 
     &                  '_naitr: Iterative refinement; rnorm1 is')
            call zvout (logfil, 1, rnorm, ndigit, 
     &                  '_naitr: Iterative refinement; rnorm is')
         end if
c
c        %----------------------------------------------------%
...
c 
         if (bmat .eq. 'G') then         
             rnorm1 = zdotc (n, resid, 1, workd(ipj), 1)
             rnorm1 = sqrt(dlapy2(dble(rnorm1),dimag(rnorm1)))
         else if (bmat .eq. 'I') then
...
c        %-----------------------------------------%
c
         if (abs(rnorm1) .gt. 0.717*abs(rnorm)) then
c
c           %---------------------------------------%
...
c 
         if (msglvl .gt. 3) then
            call zmout (logfil, j, j, h, ldh, ndigit, 
     &                  '_naitr: current upper Hessenberg matrix H')
         end if
         if (msglvl .gt. 4) then
            call zmout (logfil, n, j, v, ldv, ndigit,
...
c
         if (msglvl .gt. 3) then
            call zgemv ('C', n, j, one, v, ldv, workd(ipj), 
     &                       1, zero, workd(irj), 1)
            if (dlapy2(dble(rnorm),dimag(rnorm)) .ge. unfl) then
                temp1 = one/rnorm
                call zscal(j, temp1, workd(irj), 1)
            else if (dlapy2(dble(rnorm),dimag(rnorm)).gt. 0.0) then
                call zlascl ('General', i, i, dble(rnorm), 
     &                      dble(one), j, 1, workd(irj), j, infol)
            else 
                call zscal(j, zero, workd(irj), 1) 
            end if
            call zvout (logfil, j, workd(irj), ndigit, 
