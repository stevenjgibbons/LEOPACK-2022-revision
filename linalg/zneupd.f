...
c
c  Two of these parameters (V and INFO) are also output parameters:
c
...
c
c\Routines called:
c     zhseqr  LAPACK routine that computes the Schur form of a
...
c     zdscal  Level 1 BLAS that scales a vector.
...
c
c\SCCS Information: @(#)
c FILE: neupd.F   SID: 2.1   DATE OF SID: 11/21/95   RELEASE: 2
c
c\EndLib
c
c-----------------------------------------------------------------------
...
c
     ...
     &           zunm2r, ztrmm, zvout, zngets, ivout
c  
c     %--------------------%
...
c
     ...
     &        '_neupd: Ritz values from _neigh.')
...
c     
     ...
     &        '_neupd: Corresponding B-norm of the residual')
         call zvout (logfil, ncv, workl(ritz), ndigit,
     &        '_neupd: The Ritz values from _naupd')
         call zvout (logfil, ncv, workl(bounds), ndigit, 
     &        '_neupd: Ritz estimates of the NCV Ritz values')
         if (msglvl .gt. 2) then
            call zmout (logfil, ncv, ncv, workl(ih), ldh, ndigit,
...
c
         ...
         if (msglvl .gt. 2) then
            call zvout(logfil, 1, thres, ndigit,
...
c        |    estimate to see if it has converged.  If has, set    |
...
c
         ...
         do 10 j = 0, ncv-1
            select(j+1) = .false.
            if (which .eq. 'LM') then
               if ( abs(workl(irz+j)) .ge. thres ) then
                  if ( abs(workl(ibd+j)) .le. 
     &                 tol*abs(workl(irz+j-1)) )
     &               select(j+1) = .true.
               end if 
            else if (which .eq. 'SM') then
               if ( abs(workl(irz+j)) .le. thres )  then
                  if ( abs(workl(ibd+j)) .le. 
     &                 tol*abs(workl(irz+j-1)) )
     &               select(j+1) = .true.
               end if
...
c
            kv(1) = ktrord
            kv(2) = nconv 
            call ivout(logfil, 2, kv, ndigit,
     &        '_neupd: Number of specified vs. "converged" eigenvalues')
         end if 
c
...
c
c        %-------------------------------------------------------%
c        | Call LAPACK routine zhseqr to compute the Schur form  |
...
c
         ...
         call zhseqr ('S', 'I', ncv, 1, ncv, workl(iuptri), ldh,
     &        workl(iheig), workl(invsub), ldq, workev, lworkl,
     &        ierr)
...
c
         if (msglvl .gt. 4) then
            call zmout (logfil, ncv, nconv, workl(invsub), ldq, ndigit,
...
c
     ...
     &                 workl(invsub+(j-1)*ldq), 1 )
                  workl(ihbds+j-1) = dcmplx(
     &                               abs(workl(invsub+ncv-1+(j-1)*ldq))
     &                               ) 
 40         continue
c
            if (msglvl .gt. 2) then
...
c
            call zunm2r ('Right', 'Notranspose', n, ncv, nconv,
     &           workl(invsub), ldq, workev, z, ldz, workd(n+1), 
     &           ierr)
c
...
c
         if (rvec) then
            ...
            call zcopy (ncv, workl(bounds), 1, workl(ihbds), 1)
         end if 
c      
      else
c     
...
c
         do 100 j=1, nconv
            if (workl(iheig+j-1) .eq. zero) then
