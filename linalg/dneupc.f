...
c
      rnorm = workl(ih+2)
c      call dmout (6, ncv, 3, workl(ritzr), ldh, 10,
c     &   '_neupc: Ritz values and estimates ')
c      print *, ' RNORM ', rnorm, ' sigma ',sigma,' mu ',mu
      workl(ih+2) = zero
c     
      if (rvec) then
c     
...
c     
         ...
         call dcopy(ncv, workl(invsub+ncv-1), ldq, workl(ihbds), 1)
c      call dmout (6, ncv, 3, workl(iheigr), ldh, 15,
c     &   '_neupc: Ritz values and estimates of the Schur form ')
c
c        %----------------------------------------------------%
...
c         call dvout (6, ncv, workl(ihbds), 15,
c     &              '_neupc: last row of Schur after factor')
...
c     
     ...
     &           ierr)
c
c            call dmout (6, ncv, ncv, workl(invsub), ldq, 5,
c     &              '_neupc: upper triangular eigenvector matrix')
            if (ierr .ne. 0) then
                info = -9
                go to 9000
            end if
...
c
            ...
            call dvout (6, ncv, workl(ihbds), 15,
     &              '_neupc: last row of eigenvectors')
c
c           %---------------------------------------------------------%
...
c      call dmout (6, ncv, 3, workev, ldh, 15,
c     &   '_neupc: Ritz estimates and values (workev) ')
c
            call dscal (ncv, rnorm, workl(ihbds), 1)
            call dvout (6, ncv, workl(ihbds), 15,
