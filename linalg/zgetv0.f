...
c
c  RNORM   Complex*16 scalar.  (OUTPUT)
...
c
c\SCCS Information: @(#)
c FILE: getv0.F   SID: 2.1   DATE OF SID: 11/21/95   RELEASE: 2
c
c\EndLib
c
c-----------------------------------------------------------------------
...
c     %----------------------%
c
      external    zscal, zcopy, zgemv, zlarnv, zvout, second
c
c     %--------------------%
...
c 
      first = .FALSE.
      if (bmat .eq. 'G') then
          rnorm0 = zdotc (n, resid, 1, workd, 1)
...
c
      if (msglvl .gt. 2) then
          call zvout (logfil, 1, rnorm0, ndigit, 
     &                '_getv0: re-orthonalization ; rnorm0 is')
          call zvout (logfil, 1, rnorm, ndigit, 
     &                '_getv0: re-orthonalization ; rnorm is')
      end if
c
      if (abs(rnorm) .gt. 0.717*abs(rnorm0)) go to 50
c 
      iter = iter + 1
      if (iter .le. 1) then
c
