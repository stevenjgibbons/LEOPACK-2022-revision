...
c 
c\SCCS Information: @(#)
c FILE: naupd.F   SID: 2.1   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c
c\EndLib
...
c     %----------------------%
c
      external   znaup2, zscal, zvout, ivout, second, zstatn
c
c     %--------------------%
...
c
         if (nb .le. 0)				nb = 1
         if (tol .le. 0.0 )			tol = dlamch('EpsMach')
         if (ishift .ne. 0  .and.  
     &       ishift .ne. 1  .and.
...
c        %-----------------------------%
c
         call zscal ( 3*ncv**2 + 6*ncv, zero, workl, 1 )
c 
c        %-------------------------------------------------------------%
...
c
 ...
 1100    format (
     ...
     &      5x, 'Total time in cnaup2 routine               = ', f12.6,/
