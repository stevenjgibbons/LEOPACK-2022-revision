...
c     
     ...
     &        '_neupd: Corresponding B-norm of the residual')
         call dvout (logfil, ncv, workl(ritzr), ndigit,
     &        '_neupd: Real part of the Ritz values from _naupd')
         call dvout (logfil, ncv, workl(ritzi), ndigit,
     ...
     &        '_neupd: Ritz estimates of the NCV Ritz values')
         if (msglvl .gt. 1) then
            call dmout (logfil, ncv, ncv, workl(ih), ldh, ndigit,
...
c
     ...
     &            .ge. thres) then
                  temp1 = dlapy2( workl(irr+j), workl(iri+j) )
...
c
         if (msglvl .gt. 4) then
            call dmout (logfil, ncv, nconv, workl(invsub), ldq, ndigit,
...
c
         if (rvec) then
            ...
            call dcopy (ncv, workl(bounds), 1, workl(ihbds), 1)
         end if
c     
      else 
c     
