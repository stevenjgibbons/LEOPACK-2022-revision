...
c          On INPUT:  NP contains the number of implicit shifts at each
c          Arnoldi iteration.  This is adjusted dynamically at each 
c          iteration to accelerate convergence and prevent stagnation.
c          This is also roughly equal to the number of matrix-vector 
c          products (involving the operator OP) per Arnoldi iteration.
c          On OUTPUT: NP contains the number of "converged" eigenvalues
c          which could be greater than the no. of requested eigenvalues
c          NEV in order to keep a conjugate eigenpair together.
c
...
c 
c\SCCS Information: @(#)
c FILE: naup2.F   SID: 2.1   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c     1. None
c
...
c
c        %--------------------------------------------%
c        | Now go compute the first NEV Arnoldi steps |
c        %--------------------------------------------%
c
         ido  = 0
         if (msglvl .gt. 0) then
...
c
         if (msglvl .gt. 1) then
            call zvout (logfil, 1, rnorm, ndigit, 
...
c        %--------------------------------------------------%
c
         call zcopy(kplusp,ritz,1,workl(kplusp+1),1)
         call zcopy(kplusp,bounds,1,workl(2*kplusp+1),1)
c
c        %---------------------------------------------------%
c        | Select the wanted Ritz values and their bounds    |
c        | to be used in the convergence test.               |
...
c
         do 25 i = 1, nev
           if (abs(bounds(np+i)) .le. tol*abs(ritz(np+i))) 
     &                                             nconv = nconv + 1
   25    continue
c 
         if (msglvl .gt. 2) then
...
c
            if (msglvl .gt. 2) then 
               call zvout( logfil, kplusp, workl, ndigit,
     &              '_naup2: Ritz values from _neigh.')
               call zvout( logfil, kplusp, workl(kplusp+1), ndigit,
     &              '_naup2: Ritz estimates from _neigh.')
            end if
c
            nswap = min ( nconv, kplusp-nconv )
            if ( nconv .gt. 0 ) then
               do 1111 i = 1,ldq
                  call zcopy(kplusp,workl(kplusp+1),1,ritz,1)
                  call zcopy(kplusp,q(i,1),ldq,workl,1)
                  call zsortc (which, .true., kplusp, ritz,workl)
                  call zswap (nswap, workl(kplusp-nswap+1), 1, workl, 1) 
                  call zcopy(kplusp,workl,1,q(i,1),ldq)
 1111          continue 
               call zcopy(kplusp,workl(kplusp+1),1,ritz,1)
               call zcopy(kplusp,workl(2*kplusp+1),1,bounds,1)
               call zsortc (which, .true., kplusp, ritz, bounds)
               call zswap (nswap, ritz(kplusp-nswap+1), 1, ritz, 1) 
               call zswap (nswap, bounds(kplusp-nswap+1), 1, bounds, 1)
c
            end if
c
c           %------------------------------------------%
...
c
     ...
     &        '_naup2: Number of "converged" Ritz values')
         call zvout (logfil, nconv, ritz, ndigit,
     &        '_naup2: Values of the "converged" Ritz values')
         call zvout (logfil, nconv, bounds, ndigit,
     &        '_naup2: Associated Ritz estimates')
         if (msglvl .gt. 2) then
            call zvout (logfil, kplusp-np, ritz(np+1), ndigit,
     &           '_naup2: Values of the remaining Ritz values')
            call zvout (logfil, kplusp-np, bounds(np+1), ndigit,
     &           '_naup2: Associated Ritz estimates')
         end if 
      end if
c
