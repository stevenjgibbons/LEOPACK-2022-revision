...
c
c\SCCS Information: @(#)
c FILE: ngets.F   SID: 2.1   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c     1. xxxx
c
...
c
      if (which .eq. 'LM') then
         call zsortc ('LR', .true., kev+np, ritz, bounds)
      else if (which .eq. 'SM') then
         call zsortc ('SR', .true., kev+np, ritz, bounds)
      else if (which .eq. 'LR') then
         call zsortc ('LM', .true., kev+np, ritz, bounds)
      else if (which .eq. 'SR') then
         call zsortc ('SM', .true., kev+np, ritz, bounds)
