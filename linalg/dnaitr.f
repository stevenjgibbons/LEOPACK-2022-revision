...
c 
         if (msglvl .gt. 4) then
            call dmout (logfil, j, j, h, ldh, ndigit, 
     &                  '_naitr: current upper Hessenberg matrix H')
         end if
         if (msglvl .gt. 5) then
           call dmout (logfil, n, j, v, ldv, ndigit,
...
c
         if (msglvl .gt. 3) then
            call dgemv ('T', n, j, one, v, ldv, workd(ipj), 
     &                       1, zero, workd(irj), 1)
            if (rnorm .ge. unfl) then
                temp1 = one/rnorm
                call dscal(j, temp1, workd(irj), 1)
            else if (rnorm .gt. zero) then
                call dlascl ('General', i, i, rnorm, one, j, 1, 
     &                      workd(irj), j, infol)
            else 
                call dscal(j, zero, workd(irj), 1) 
            end if
            call dvout (logfil, j, workd(irj), ndigit, 
