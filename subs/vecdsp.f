C*********************************************************************
C subroutine VECtor DiSPlay ******************************************
C            ---    - --    ******************************************
C Steve Gibbons 16.3.99                                              C
C____________________________________________________________________C
C                                                                    C
C Outputs an eye-readable breakdown of the solution vector with      C
...
C
C*********************************************************************
      SUBROUTINE VECDSP ( NDIM, NR, NH, MHT, MHL, MHM, MHC, RI, RO,
     1                    RVEC, LU, IMODE, IZF )
      IMPLICIT NONE
C____________________________________________________________________C
...
C
      IF ( NDIM.NE.NR*NH ) THEN
         PRINT *,' Subroutine VECDSP. Bad dimensions '
...
C
     ...
     1       CH11, MHL( IH ), MHM( IH ), CH3
        ...
        DO IRN = 1, NR
           RAD = RI + H*( IRN - 1 )
           IND = ( IRN - 1 )*NH + IH
           IF ( IMODE.EQ.1 ) WRITE ( LU, 90 ) RAD, RVEC( IND )
        ENDDO
 100    CONTINUE
      ENDDO
