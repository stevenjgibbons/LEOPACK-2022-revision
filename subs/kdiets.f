C*********************************************************************
C subroutine Kinematic Dynamo Induction Equation Time Step ***********
C            -         -      -         -        -    -    ***********
C Steve Gibbons Mon Dec 18 11:07:29 WET 2000                         C
...
C                                                                    C
C The array FDCM is not called by KDIETS directly, but must be       C
...
C
C*********************************************************************
      SUBROUTINE KDIETS( ML4,MM4,IP4,ML5,MM5,IP5,AM4,BM4,AM5,BM5,
     1 SV4,SV5,D4,D5,E4,E5,R4,R5,PA,DPA,F1,F2,F3,XSV,GAUX,GAUW,SV4Q,
     2 SV4S,SV5T,VQ4,VS4,VT5,VT4,VQ5,VS5,CT4P,CQ5T,CS5T)
      IMPLICIT NONE
C____________________________________________________________________C
...
C
      ...
      ENDIF
C
      IF ( LW ) WRITE ( IOUTF, * ) 'Entered KDIETS..'
C
...
C
     ...
     1             BETA, D4, INCY )
      IF ( LW ) WRITE ( IOUTF, * )
     1    'KDIETS: Called DGBMV: dv4:= bm4 . sv4.'
C
...
C
     ...
     1             BETA, D5, INCY )
      IF ( LW ) WRITE ( IOUTF, * )
     1    'KDIETS: Called DGBMV: dv5:= bm5 . sv5.'
C
...
C
      ...
      ICMPHI = ICPHI0
C
      CALL IENLTF( ML4,MM4,ML5,MM5,SV4,SV5,R4,R5,PA,DPA,F1,F2,F3,
     1             XSV,GAUX,GAUW,SV4Q,SV4S,SV5T,VQ4,VS4,VT5,VT4,VQ5,VS5,
...
C
      ...
      ICMPHI = ICPHI1
C
      CALL IENLTF( ML4,MM4,ML5,MM5,E4,E5,R4,R5,PA,DPA,F1,F2,F3,
     1             XSV,GAUX,GAUW,SV4Q,SV4S,SV5T,VQ4,VS4,VT5,VT4,VQ5,VS5,
     2             CT4P,CQ5T,CS5T,ICMRAD,ICMTHE,ICMPHI )
C
      N = NR*NH4
...
C
      IF ( LW ) WRITE ( IOUTF, * )
     1    'KDIETS: Iteration ',NOIT,' norm = ', AVEC1
C
      DIFF = DABS( AVEC1 - AOLD )
      IF ( DIFF.LT.DTOL ) THEN
        IF ( LW ) WRITE ( IOUTF, * )
     1    'KDIETS: Soln. converged iteration ', NOIT
          RETURN
      ENDIF
C
...
C
      IF ( DIFF.GT.ODIFF .AND. NOIT.GT.3 ) THEN
        IF ( LW ) WRITE ( IOUTF, * )
     1    'KDIETS: Soln. norm increasing: iteration ', NOIT
        NOIT = -2
        RETURN
      ENDIF
C
