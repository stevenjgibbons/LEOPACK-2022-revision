C*********************************************************************
C subroutine Double Precision Matrix curl ( Velocity . grad Vel ) ****
C            -      -         -             -               -     ****
C Steve Gibbons 16.3.99                                              C
C____________________________________________________________________C
C                                                                    C
C This routine adds to the convection matrix the terms associated    C
C with the non-linear v . grad v . This is the version which         C
C assumes that the sets of harmonics for v_0 and v_0 are             C
...
C                                                                    C
C     FACV0V    : Multiplication factor for curl ( v_0 . grad ) v    C
...
C
C*********************************************************************
      SUBROUTINE DPMVV ( N1, N2, NR, NH, KL, KU, KLE, IMF, MHT, MHL,
     1                   MHM, MHC, LH, NPHPTS, NTHPTS, RI, RO, AMAT,
     2                   FACV0V, FACVV0, GAUX, GAUW, PA, DPA,
     3                   OLDVEC, ORD, FTF1, FTF2, FTF3, VFA, VFB,
     4                   VFG, QST )
      IMPLICIT NONE
C____________________________________________________________________C
C Variable declarations - Parameters ................................C
     ...
     2        LH, NTHPTS, NPHPTS
      DOUBLE PRECISION RI, RO, AMAT( N1, N2 ), FACV0V, FACVV0,
...
C
      IF ( N2.NE.NH*NR ) THEN
        PRINT *,' Subroutine DPMVV.  bad array size'
...
C
      ...
      OVV0 = .TRUE.
C
      IF ( ABS( FACV0V ).LT.TOL ) OV0V = .FALSE.
      IF ( ABS( FACVV0 ).LT.TOL ) OVV0 = .FALSE.
C
      IF ( (.NOT. OV0V) .AND. (.NOT. OVV0) ) RETURN
C
c
C     --------------------------------------------------------
...
C
               IF ( OV0V ) THEN
C
c        . Calculate curl ( v_old . Nabla ) v_new
c        . old toroidal alpha, new poloidal beta
...
c        .
c        . Calculate curl ( v_old . Nabla ) v_new
c        . old toroidal alpha, new toroidal beta
c        . output is toroidal gamma
c        .
               IF ( OV0V ) THEN
C          .
...
c        .
c        . Calculate curl ( v_old . Nabla ) v_new
c        . old poloidal alpha, new poloidal beta
c        . output is toroidal gamma
c        .
               IF ( OV0V ) THEN
C          .
...
c        .
c        . Calculate curl ( v_old . Nabla ) v_new
c        . old poloidal alpha, new toroidal beta
c        . output is toroidal gamma
c        .
               IF ( OV0V ) THEN
C          .
