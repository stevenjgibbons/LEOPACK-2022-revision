C*********************************************************************
C subroutine New Transform Solution Vector Forcing term: (aZim. sym).*
C            -   -         -        -      -               -         *
C Steve Gibbons Sat Jul 22 16:32:29 BST 2000                         *
...
C                                                                    C
C     RVF1      : Work arr. dim. ( 3, NPHPV, NTHP, NR )              C
C     RVF2      : Work arr. dim. ( 3, NPHPV, NTHP, NR )              C
C     RVF3      : Work arr. dim. ( 3, NPHPV, NTHP, NR )              C
C     SF        : Work arr. dim. ( NPHPV, NTHP )                     C
C     VF        : Work arr. dim. ( 3, NPHPV, NTHP )                  C
...
C     RQST1     : Work array. Dim ( NR, LH*(LH+2) ,3 ).              C
C     RQST2     : Work array. Dim ( NR, LH*(LH+2) ,3 ).              C
C     RQST3     : Work array. Dim ( NR, LH*(LH+2) ,3 ).              C
C     RQSTA     : Work array. Dim ( NR, LH*(LH+2) ,3 ).              C
C                                                                    C
C____________________________________________________________________C
C
C*********************************************************************
      SUBROUTINE NTSVFZ( NR, INARR, MHT, MHL, MHM, MHTR, LH, NCFM,
     1                 NBN, MMAX, M0, NTHP, NPHPR, NPHPV, CB1, CB2,
     2                 CC, CH, CG, CF, CJ, CM, V0, V1, RVF1, RVF2,
     3                 RVF3, SF, VF, PA, DPA, GAUX, GAUW, SHC, DSHC,
     4                 FDCM, RHS, XARR, RQST1, RQST2, RQST3, RQSTA )
      IMPLICIT NONE
C____________________________________________________________________C
C Variable declarations - Parameters ................................C
      INTEGER INARR( * ), MHT( * ), MHL( * ), MHM( * ), MHTR( * ),
     1        LH, NCFM, NBN, MMAX, M0, NTHP, NPHPV, NPHPR, NR
      DOUBLE PRECISION FDCM( NCFM, NR, 1 ), SF( NPHPV, NTHP ),
     1               RVF1( 3, NPHPV, NTHP, NR ), GAUX( NTHP ),
     2               RVF2( 3, NPHPV, NTHP, NR ), GAUW( NTHP ),
     3               RVF3( 3, NPHPV, NTHP, NR ), VF( 3, NPHPV, NTHP )
     ...
     1                 V1( * ), RHS( * ), XARR( * ),
     2                 RQSTA( NR, LH*(LH+2), 3 )
      DOUBLE PRECISION RQST1( NR, LH*(LH+2), 3 ),
     1                 RQST2( NR, LH*(LH+2), 3 ),
     2                 RQST3( NR, LH*(LH+2), 3 )
C____________________________________________________________________C
C Variable declarations - Working variables .........................C
C
      INTEGER ILNR, IRNR, ILNT, IRNT, ICMPI, ICMPO,
     1        NPHMAX, NRMAX, NDRVS, ICLS, IOP, IRES
      PARAMETER ( NPHMAX = 256, NRMAX = 300 )
...
C     .
      IF ( NPHPR.GT.NPHMAX .OR. NR.GT.NRMAX ) THEN
        PRINT *,' Subroutine NTSVFZ.'
        PRINT *,' NR    = ', NR,  ' NRMAX  = ', NRMAX
        PRINT *,' NPHPR = ', NPHPR,' NPHMAX = ', NPHMAX
...
C     .
      ...
      IRNR  = NR - 1
      CHVMFF = 'VEL'
      CALL SDRQSO( NR, LH, V0, V1, ILNR, IRNR, INARR,
     1             RQST1, XARR, MHT, MHL, MHM, CHVMFF )
C     .
C     . Now store the velocity in RVFO format - do this
C     . directly from the solution vector (SDVRVF).
C     .
      CALL SDVRVZ( NTHP, NPHPR, NPHPV, M0, LH, IRES, ILNR, IRNR,
     1             NR, INARR, MHT, MHL, MHM, MMAX, CHVMFF, V0, V1,
...
C     .
      FAC   = -1.0d0*CC
      CALL RVVGTZ( NR, LH, MMAX, MHT, MHL, MHM, V0, V1, INARR,
     1             MHTR, MHL, MHM, RHS, INARR, RVF1, VF, SF, FTF1,
     2             FTF2, FTF3, SHC, DSHC, GAUX, GAUW, PA, DPA, NTHP,
     3             NPHPV, NPHPR, M0, FAC, ILNR, IRNR, XARR )
C     .
...
C     .
      ...
      A     = 0.0d0     
      B     = 1.0d0     
      CALL RQSTCO( LH, NR, M0, MMAX, NBN, NCFM, NDRVS, ILNR,
     1     IRNR, ILNR, IRNR, RQST1, RQST2, XARR, FDCM, ICLS, A, B )
C     .
C     . Evaluate (curl v) in space using RQSRVO (from RQST2)
C     . Store curl v in RVF2.
C     .
      CALL VECOP( ZCFA, ZERO, NR, IOP )
      CALL RQSRVZ( RQST2, RVF2, GAUX, PA, DPA, FTF1, FTF2, FTF3,
     1             LH, NTHP, NPHPR, NPHPV, M0, MMAX, ZCFA, NR,
     2             ILNR, IRNR )
C     .
C     . Evaluate -cg( k x v) + cf[ v x curl v ] in RVF3
C     .
      CALL RVFCIT( NTHP, NPHPV, NR, ILNR, IRNR, RVF1,
     1                    RVF2, RVF3, GAUX, CG, CF )
C     .
C     . RVF3 now contains -cg( k x v) + cf[ v x curl v ].
C     . Use RVFRQO to convert this function to RQST format,
C     . storing output in RQST3.
C     .
      CALL RVFRQZ( RQST3, RVF3, GAUX, GAUW, PA, DPA, FTF1, FTF2,
     1        FTF3, ZCFA, LH, NTHP, NPHPV, NPHPR, M0, MMAX,
     2        NR, ILNR, IRNR )
C     .
...
C     .
      CALL RVFCPO( RVF1, RVF2, RVF3, NPHPV, NTHP, NR, ILNR, IRNR )
C     .
...
C     .
      FAC = 1.0d0
      CALL RQSTSO( NR, LH, ILNR, IRNR, INARR, MHTR, MHL, MHM,
...
C     .
      CALL VECOP( ZCFA, ZERO, NR, IOP )
      CALL RQSRVZ( RQST1, RVF1, GAUX, PA, DPA, FTF1, FTF2, FTF3,
     1              LH, NTHP, NPHPR, NPHPV, M0, MMAX, ZCFA, NR,
     2               ILNR, IRNR )
C     .
...
C     .
      CALL RVFRQZ( RQST3, RVF3, GAUX, GAUW, PA, DPA, FTF1, FTF2,
     1        FTF3, ZCFA, LH, NTHP, NPHPV, NPHPR, M0, MMAX, NR,
     2        ILNR, IRNR )
C     .
