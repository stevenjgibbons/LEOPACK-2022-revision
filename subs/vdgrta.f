C*********************************************************************
C subroutine Velocity Dot GRadient of Theta Add **********************
C            -        -   --          -     -   **********************
C Steve Gibbons Sat Oct  2 18:26:43 BST 1999                         C
C____________________________________________________________________C
C                                                                    C
C If the vector VI contains radial functions to NHI spherical harm.s C
C with type defined by MHTI, MHLI and MHMI, then VDGRTA will add FAC C
...
C must be obtained in advance by calling SVRQST!!                    C
C VDGRTA does NOT extract the velocity from VI. This is done         C
C entirely to reduce the programming load and ease of checking.      C
C                                                                    C
C Grad theta IS calculated by VDGRTA from the harmonics in VI with   C
C MHTI( ih ) = 3.                                                    C
C                                                                    C
C                                                                    C
C____________________________________________________________________C
...
C
C*********************************************************************
      SUBROUTINE VDGRTA ( NR, LH, MMAX, MHTI, MHLI, MHMI, VI, INARRI,
     1             MHTO, MHLO, MHMO, VO, INARRO, RQST, VF1, VF2, SF,
     2             FTF1, FTF2, FTF3, SHC, DSHC, GAUX, GAUW, PA, DPA,
     3             NTHPTS, NPHPTS, FAC, ILNR, IRNR, NBN, XARR,
     4             NFDCM, NDVDS, FDCM, ILNC, IRNC, ZCOEFA )
      IMPLICIT NONE
C____________________________________________________________________C
C Variable declarations - Parameters ................................C
C
      INTEGER NR, LH, MMAX, NTHPTS, NPHPTS, ILNR, IRNR,
     1        NBN, NFDCM, NDVDS, ILNC, IRNC,
     ...
     3                 DPA ( ( LH + 1 )*( LH + 2 )/2 , NTHPTS)
      DOUBLE PRECISION FDCM( NFDCM, NR, NDVDS ),
...
C Variable declarations - Working variables .........................C
      INTEGER NRI, NRO, NHI, NHO, IR, NH, IHI, IHARM,
     1        IND, L, M, ICS, IHD, IHO, INDSHC, INDFUN
      DOUBLE PRECISION DERV( 1 ), ZERO, D0F, D1F, DZCOEF, ZCOEF,
     1                 RAD
...
C         .
          ...
          D0F = VI( IND )
          CALL NGSVDR( VI, IR, IHI, NBN, IHD, NFDCM, NR, NDVDS,
     1                 DERV, ILNC, IRNC, INARRI, FDCM )
          D1F = DERV( 1 )
C         .
