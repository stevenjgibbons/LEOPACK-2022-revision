C*********************************************************************
C subroutine Kinematic Dynamo Induction Equation Time step (Optim) ***
C            -         -      -         -        -          -      ***
C Steve Gibbons Wed Feb 14 09:59:12 WET 2001                         C
...
C                                                                    C
C The array FDCM is not called by KDIETO directly, but must be       C
...
C
C*********************************************************************
      SUBROUTINE KDIETO( MM4,IP4,MM5,IP5,AM4,BM4,AM5,BM5,
...
C
      INTEGER          MM4( NH4 ), IP4( NH4*NR )
      INTEGER          MM5( NH5 ), IP5( NH5*NR )
C
...
C
      ...
      ENDIF
C
      IF ( LW ) WRITE ( IOUTF, * ) 'Entered KDIETO.'
C
...
C
     ...
     1             BETA, D4, INCY )
      IF ( LW ) WRITE ( IOUTF, * )
     1    'KDIETO: Called DGBMV: dv4:= bm4 . sv4.'
C
...
C
     ...
     1             BETA, D5, INCY )
      IF ( LW ) WRITE ( IOUTF, * )
     1    'KDIETO: Called DGBMV: dv5:= bm5 . sv5.'
C
...
C
      ...
      ICMPHI = ICPHI0
C
      CALL IENLTO(     MM4,    MM5,SV4,SV5,R4,R5,       F1,F2,F3,
...
C
      ...
      ICMPHI = ICPHI1
C
      CALL IENLTO(     MM4,    MM5,E4,E5,R4,R5,       F1,F2,F3,
     1             XSV,          SV4Q,SV4S,SV5T,VQ4,VS4,VT5,VT4,VQ5,VS5,
     2             CT4P,CQ5T,CS5T,ICMRAD,ICMTHE,ICMPHI,
     3             FTF1P4,FTF2P4,FTF3P4,FTF2T5,FTF3T5,
     4             QF1A5,SF2A5,SF3A5,TF2A4,TF3A4 )
...
C
      IF ( LW ) WRITE ( IOUTF, * )
     1    'KDIETO: Iteration ',NOIT,' norm = ', AVEC1
C
      DIFF = DABS( AVEC1 - AOLD )
      IF ( DIFF.LT.DTOL ) THEN
        IF ( LW ) WRITE ( IOUTF, * )
     1    'KDIETO: Soln. converged iteration ', NOIT
          RETURN
      ENDIF
C
...
C
      IF ( DIFF.GT.ODIFF .AND. NOIT.GT.3 ) THEN
        IF ( LW ) WRITE ( IOUTF, * )
     1    'KDIETO: Soln. norm increasing: iteration ', NOIT
        NOIT = -2
        RETURN
      ENDIF
C
