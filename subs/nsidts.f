...
C SFDDMF. NSIDTS will advance the solution by one time step (size    C
C DELTAT).                                                           C
...
C
C*********************************************************************
      SUBROUTINE NSIDTS( KL, NBN, NCFM, NR, NDRVM, NH, NDCS, MHT, MHL,
     ...
     5   RQSTA, NTS, NNDS, IW1, W1, W2, W3, U, V )
      IMPLICIT NONE
C____________________________________________________________________C
...
C
 ...
 81   FORMAT('nsidts: Predictor norm = ',1PD16.7)
C
      ITMX = NOIT
      NOIT = 0
 50   CONTINUE
 ...
 83     FORMAT('nsidts: Max. iterations exceeded.')
        NOIT = -1
        RETURN
      ENDIF
C
...
C
 ...
 84   FORMAT('nsidts: Iteration ',I4,' Solution norm = ',1PD16.7)
