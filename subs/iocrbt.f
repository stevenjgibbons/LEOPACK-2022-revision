C*********************************************************************
C subroutine Instability Onset Critical Rayleigh Bounds refine *******
C            -           -     -        -        -             *******
C                                         Two matrix version.  *******
C                                         -                    *******
C Steve Gibbons Mon Dec 13 10:56:32 GMT 1999                         C
...
C                                                                    C
C Differs from IOCRBR in that the non-linear terms are calculated    C
C in a matrix B ( of dimensions [ 2*kl + 1, n2 ] ) and then          C
C copied into A ( of dimensions [ 3*kl + 1, n2 ] ) for solution      C
C with the inclusion of the buoyancy term.                           C
C                                                                    C
C This is done in order to save time - not having to repeat the      C
C calculation of the non-linear terms over and over again.           C
C                                                                    C
C The disadvantage is that it requires more memory.                  C
C____________________________________________________________________C
C                                                                    C
C  Integer                                                           C
C  -------                                                           C
...
C                                                                    C
C     N1A       : First dimension of matrix, A.                      C
C     N1B       : First dimension of matrix, B.                      C
...
C                 LULOG = 144, only CH, GRR and GRI are written      C
C                   out at each iteration.                           C
C                 LULOG = 145, full eigensystem written out.         C
C                                                                    C
...
C                                                                    C
C     A         : Matrix. Dim ( N1A, N2 )                            C
C     B         : Matrix. Dim ( N1B, N2 )                            C
...
C
C*********************************************************************
      SUBROUTINE IOCRBT( NR,INARR,MHT,MHL,MHM,MHP,MHTR,NBN,KL,NCFM,
     1   NDRVM,N1A,N1B,N2,NDCS,NTHP,NPHP,MMAX,LH,IEV,MHIBC,MHOBC,
     2   NEV,NCV,NCVM,MXIT,IPIV,LULOG,SVFDC,A,B,XARR,GAUX,GAUW,PA,
     3   DPA,SBRVEC,RESID,W2,WVEC,F1,F2,F3,VF1,VF2,VF3,QST,SF,SHC,
     4   SELECT,ARTOL,DRSV,PARAM,MXATT,IERR,DR,DI,D3,WORKEV,WORKD,
     5   WORKL,V,IN0,MT0,ML0,MM0,MP0,VEC0 )
      IMPLICIT NONE
C____________________________________________________________________C
C Variable declarations - Parameters ................................C
      INTEGER NR, INARR( 3 ), MHT( * ), MHL( * ), MHM( * ), MHP( * ),
     1        MHTR( * ), NBN, KL, NCFM, NDRVM, N1A, N1B, N2, NDCS,
     2        MHIBC( NDCS ), MHOBC( NDCS ), NEV, NCV, NCVM, MXIT,
     3        IPIV( * ), LULOG, MMAX, LH, IEV, NTHP, NPHP, MXATT,
     4        IERR, IN0( * ), MT0( * ), ML0( * ), MM0( * ), MP0( * )
      DOUBLE PRECISION A( N1A, N2 ), B( N1B, N2 ), SBRVEC( * ),
     1                 DR( NCVM ), DI( NCVM ), D3( NCVM ), DRSV,
     2                 WORKEV( 3*NCVM ), WORKD( 3*N2 ), XARR( NR ),
...
C
     ...
     4                 FACA, FACB
      PARAMETER ( DIAGEL = -200.0d0, DELTA = 1.0d-8,
     1            ZERO = 0.0d0 )
      LOGICAL OSBR
C____________________________________________________________________C
...
C
      IF ( CTOL.LT.ZERO ) THEN
        PRINT *,' Subroutine IOCRBT.'
        PRINT *,' CTOL = ', CTOL
        PRINT *,' Program aborted.'
        STOP
      ENDIF
C
      IF ( LULOG.NE.0 .AND. LULOG.NE.144 .AND. LULOG.NE.145 ) THEN
        PRINT *,' Subroutine IOCRBT.'
        PRINT *,' LULOG = ', LULOG
        PRINT *,' Should be 0, 144 or 145.'
        PRINT *,' Program aborted.'
        STOP
      ENDIF
C
C Check first dimensions of matrices A and B
C
      IF ( N1A.NE.(3*KL+1) ) THEN
        PRINT *,' Subroutine IOCRBT'
        PRINT *,' N1A = ', N1A
        PRINT *,' KL  = ', KL
        PRINT *,' Program aborted.'
...
C
      ...
      ENDIF
C
C Set up matrix B with the Rayleigh number invariant terms
C
      ICLS = 1
      PARAM(  1 ) = ZERO
      PARAM(  2 ) = CB1
...
C
     ...
     1          NCFM, NDRVM, SVFDC, B, N1B, N2, NDCS, XARR, NTHP,
...
C
      ...
      ENDIF
C
C Copy non-linear terms into matrix.
C
      FACA = ZERO
      FACB = 1.0d0
      CALL BMA2BM( KL, N2, A, B, FACA, FACB )
C
      ICLS = 0
      PARAM(  1 ) = ZERO
      PARAM(  2 ) = ZERO
...
C Add buoynacy to the linear stability matrix
C
      CALL AVMATA( NR, INARR, MHT, MHL, MHM, MHP, MHTR, NBN, KL,
     1          NCFM, NDRVM, SVFDC, A, N1A, N2, NDCS, XARR, NTHP,
     2          NPHP, MMAX, LH, GAUX, GAUW, PA, DPA, F1, F2, F3,
     3          VF1, VF2, VF3, QST, SF, SHC, PARAM, ICLS,
     4          IN0, MT0, ML0, MM0, MP0, VEC0 )
C
...
C
     ...
     1       NCFM, NDRVM, SVFDC, A, N1A, N2, NDCS, XARR, OSBR, SBRVEC,
...
C
      IF ( DSRSV.NE.DRSV ) THEN
        DRSV = DSRSV
        IF ( LULOG.EQ.145 ) WRITE ( LULOG, 19 ) DSRSV
      ENDIF
