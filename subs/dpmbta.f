C*********************************************************************
C subroutine Double Precision Matrix Buoyancy Term Add ***************
C            -      -         -      -        -    -   ***************
C Steve Gibbons 16.3.99                                              C
C____________________________________________________________________C
C Adds the term to the double precision matrix resulting from        C
C the curl ( theta rvec ) term in teh momentum equation.             C
C                                                                    C
C____________________________________________________________________C
C Input variables :-                                                 C
C ===============                                                    C
...
C
C*********************************************************************
      SUBROUTINE DPMBTA ( N1, N2, NR, NH, KL, KU, KLE, IMF, MHT,
...
C Variable declarations - Working variables .........................C
      CHARACTER *(2) ORD
      EXTERNAL LAPTS
      INTEGER IIH, IOH, LI, MI, ICSI, INTPAR( 2 )
...
C
      IF ( N2.NE.NH*NR ) THEN
        PRINT *,' Subroutine DPMBTA, bad array size'
...
C
      ORD = 'SS'
C
C Loop around the temperature harmonics and seek
C the equivalent toroidal harmonics
C
      DO IIH = 1, NH
        IF ( MHT( IIH ).EQ.3 ) THEN
c
c       . we have a temperature harmonic
c       . so loop around to find equivalent
c       . toroidal harmonic. However, this
c       . in effect means finding a poloidal
c       . harmonic.
c
         LI   = MHL( IIH )
         MI   = MHM( IIH )
