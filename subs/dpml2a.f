C*********************************************************************
C subroutine Double Precision Matrix Laplacian 2 (toroidal) Add ******
C            -      -         -      -         -            -   ******
...
C
C*********************************************************************
      SUBROUTINE DPML2A ( N1, N2, NR, NH, KL, KU, KLE, IMF, MHT,
...
C
      IF ( N2.NE.NH*NR ) THEN
        PRINT *,' Subroutine DPML2A, bad array size'
...
C
C Loop around the toroidal velocity harmonics and seek
C the equivalent poloidal harmonics
C
      DO IIH = 1, NH
        IF ( MHT( IIH ).EQ.2 ) THEN
c
c       . we have a toroidal velocity harmonic
