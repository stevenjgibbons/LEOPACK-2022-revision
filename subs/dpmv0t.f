...
C     OLDVEC    : Vector containing (v_0, theta_0 ). Dimen. ( LVEC ) C
C                                                                    C
C  Character                                                         C
C  ---------                                                         C
...
C
C*********************************************************************
      SUBROUTINE DPMV0T ( N1, N2, LVEC, NR, NOH, NNH, KL, KU, KLE,
...
C Variable declarations - Parameters ................................C
     ...
     3        LH, NTHPTS, NPHPTS
      DOUBLE PRECISION RI, RO, AMAT( N1, N2 ), FACV0T,
     ...
     2                 THEVF( NPHPTS, NTHPTS, 3 )
      DOUBLE PRECISION OLDVEC( LVEC ), SHC( LH*(LH+2) ),
     1                 SF( NTHPTS, NPHPTS )
      CHARACTER *(2) ORD
C____________________________________________________________________C
...
C
      IF ( N2.NE.NNH*NR ) THEN
        PRINT *,' Subroutine DPMV0T, bad array size'
...
C
      IF ( LVEC.NE.NOH*NR ) THEN
        PRINT *,' Subroutine DPMV0T, bad vector length'
        PRINT *,' LVEC = ',LVEC
        PRINT *,' NOH = ',NOH,' NR = ',NR
        STOP
      ENDIF
...
C
      IF ( ABS( FACV0T ).LT.TOL ) RETURN
C
      DO IBETA = 1, NNH
         IF (     MNHT( IBETA ).EQ.3      .AND.
     1            MNHL( IBETA ).EQ.0      .AND.
     2            MNHM( IBETA ).EQ.0      .AND.
     3            MNHC( IBETA ).EQ.1            ) THEN
C
...
C
        ...
        ENDIF
      ENDDO
C     .     end IBETA = 1, NNH (for ibeta = theta_0^0 ....
...
C        ............................................................
      ...
      DO IBETA = 1, NNH
        IF ( MNHT( IBETA ).EQ.3 .AND. MNHL( IBETA ).NE.0 ) THEN
         LB   = MNHL( IBETA )
         MB   = MNHM( IBETA )
         ICSB = MNHC( IBETA )
