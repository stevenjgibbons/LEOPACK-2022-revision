...
C                                                                    C
C If RQSTO contains a QST decomposition of a vector function         C
C then CRQSTO is returned containing                                 C
C                                                                    C
C  A * CRQSTO + B * curl ( RQSTO )                                   C
C                                                                    C
...
C                                                                    C
C     CRQSTO    : Dim ( NR, LH*(LH+2), 3 ) - Is returned containing  C
...
C
C*********************************************************************
      SUBROUTINE RQSTCO( LH, NR, M0, MMAX, NBN, NFDCM, NDVDS, ILNR,
     1                   IRNR, ILNC, IRNC, RQSTO, CRQSTO, XARR, FDCM,
     2                   ICLS, A, B )
      IMPLICIT NONE
C____________________________________________________________________C
C Variable declarations - Parameters ................................C
     ...
     2                 RQSTO( NR, LH*(LH+2), 3 ),
     3                 CRQSTO( NR, LH*(LH+2), 3 )
C____________________________________________________________________C
...
C     .
      IF ( M0.EQ.0 ) THEN
        PRINT *,' Subroutine RQSTCO.'
        PRINT *,' M0 = ', M0
        PRINT *,' Program aborted.'
        STOP
      ENDIF
C     .
C     . First zero all elements of CRQSTO which correspond
...
C       .
        DO IR = ILNR, IRNR
          RAD = XARR( IR )
          D0F = RQSTO( IR, IH, IQ )
          CRQSTO( IR, IH, IT ) = A*CRQSTO( IR, IH, IT ) -
...
C       .
        DO IR = ILNR, IRNR
          RAD = XARR( IR )
          D0F = RQSTO( IR, IH, IS )
          CALL RQSTDO( RQSTO, IR, IH, NBN, IHD, NFDCM, NR, NDVDS,
     1                 DERV, ILNC, IRNC, FDCM, LH, IS )
          D1F = DERV( 1 )
          CRQSTO( IR, IH, IT ) = CRQSTO( IR, IH, IT ) +
     1                                   B*( D1F + D0F/RAD )
        ENDDO
C       .
...
C       .
     ...
     1                 DERV, ILNC, IRNC, FDCM, LH, IT )
          D1F = DERV( 1 )
          CRQSTO( IR, IH, IQ ) = A*CRQSTO( IR, IH, IQ ) -
     1                          B*SQRLL1( L )*D0F/RAD
          CRQSTO( IR, IH, IS ) = A*CRQSTO( IR, IH, IS ) -
     1                          B*( D1F + D0F/RAD )
        ENDDO
C       .
 50   CONTINUE
