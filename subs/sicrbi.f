...
C            -     -                 -        -        -             *
C            optimised with Solid body rotation                      C
C                           -                                        C
C Steve Gibbons Mon Oct 15 13:21:49 WEST 2001                        C
...
C     VEC0W     : Work array, same dimensions as VEC0.               C
C                                                                    C
C     REYSB1    : First solid body rotation to be applied.           C
C     REYSB2    : Second solid body rotation to be applied.          C
C                                                                    C
C     Final solid body rotation is applied in REYSB1                 C
C                                                                    C
C  Logical                                                           C
C  -------                                                           C
C                                                                    C
...
C
C*********************************************************************
      SUBROUTINE SICRBI( NR,INARR,MHT,MHL,MHM,MHP,MHTR,NBN,KL,NCFM,
     ...
     5   IN0,MT0,ML0,MM0,MP0,VEC0,VEC0W,M0,REYSB1,REYSB2)
      IMPLICIT NONE
C____________________________________________________________________C
...
C
      INTEGER          NCE, ICLS, IH, NOIT, ITGN, NOITM
      PARAMETER      ( NOITM = 30 )
      DOUBLE PRECISION DWORK( 2, NOITM ), F1X, F2X, F3X, X1X, X2X,
     1                 X3X, RETM, RETC, REYSBR
...
C____________________________________________________________________C
C
      IF ( MXATT.GT.NOITM ) THEN
        PRINT *,' Subroutine SICRBI.'
        PRINT *,' MXATT = ', MXATT,' NOITM = ', NOITM
...
C
      IF ( NOIT.EQ.3 ) THEN
        REYSBR = 0.5d0*( DWORK( 1, 1 ) + DWORK( 1, 2 ) )
      ENDIF
C
      IF ( NOIT.GT.3 ) THEN
        X1X = DWORK( 1, NOIT - 3 )
        ...
        X3X = DWORK( 1, NOIT - 1 )
        F3X = DWORK( 2, NOIT - 1 )
        CALL TPAQVF( X1X, F1X, X2X, F2X, X3X, F3X, RETC, RETM, IERR )
        IF ( IERR.EQ.1 ) THEN
          PRINT *,' Subroutine SICRBI.'
          PRINT *,' TPAQVF failed to find a value for REYSBR.'
          PRINT *,' Program aborted.'
          STOP
        ELSE
          REYSBR = RETC
...
C
      ...
      ENDIF
C
      CALL SBRVMR( IN0, MT0, ML0, MM0, VEC0, VEC0W, XARR,
     1             REY, REYSBR )
C
...
C
     ...
     4          IN0, MT0, ML0, MM0, MP0, VEC0W, M0 )
C
