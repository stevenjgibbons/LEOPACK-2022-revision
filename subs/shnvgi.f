...
C                           -      -            -    -               *
C                    Gradient of Inhomogeneous temperature function. *
C                    -           -                                   *
C Steve Gibbons Wed Jan 26 09:49:34 GMT 2000                         C
...
C SHNVGI will evaluate the necessary radial functions and their      C
...
C                                                                    C
C Differs from SHNVGI in that the temperature, T is given by         C
C                                                                    C
C Theta + f( r ) where                                               C
C                                                                    C
...
C     CA        : Coefficient of inhomog. temp. (see above)          C
C     CB        : Coefficient of inhomog. temp. (see above)          C
C     CC        : Coefficient of inhomog. temp. (see above)          C
C                                                                    C
C  Character                                                         C
C  ---------                                                         C
C                                                                    C
...
C
C*********************************************************************
      FUNCTION SHNVGI( IR, RAD, NBN, TYPE, NR, NDRVS, NDRVM, NFDCM,
     1                 NDCS, SVFDC, LA, VECA, IHA, ISA, INARRA, LB,
     2                 VECB, IHB, ISB, INARRB, RI, RO, CA, CB, CC )
      IMPLICIT NONE
C____________________________________________________________________C
...
C       Now add on inhomogeneous temperature term
C       .
        CALL ITFA( RAD, RI, RO, CA, CB, CC, WORKB, IHDB )
C       .
        SHNVGI = FLA*FLA*WORKA( 1 )*WORKB( 2 )/RAD
        RETURN
C       .
      ENDIF
C     .
...
C       .
        ...
        SHNVGI = FLA*FLB*TEMP*WORKB( 1 )/RAD
        RETURN
C       .
      ENDIF
C     .
...
C       .
        CALL ITFA( RAD, RI, RO, CA, CB, CC, WORKB, IHDB )
C       .
        SHNVGI = (-1.0d0)*FLA*FLB*WORKA( 1 )*WORKB( 1 )/RAD
        RETURN
C       .
      ENDIF
C     .
