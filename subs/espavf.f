C*********************************************************************
C subroutine Equatorial Section Polar Array Value Find ***************
C            -          -       -     -     -     -    ***************
C Steve Gibbons Tue Jul 11 10:37:53 BST 2000                         C
C____________________________________________________________________C
C                                                                    C
C Given NRAD values of R in the array RVALS and NPHI values of       C
C phi in the array PVALS (given in RADIANS!), ESPAVF will fill       C
C the array ESPAV( dim[ NRAD, NPHI ]) with values corresponding to   C
C a given component of the solution. Now it is customary for         C
C RVALS( 1 ) to be zero; and all j with RVALS( j ).lt.XARR( 1 )      C
C will have ESPAV filled with VMISS                                  C
C                                                                    C
...
C                                                                    C
C     ESPAV     : Dim ( NRAD, NPHI) output with evaluated functions. C
C____________________________________________________________________C
C
C*********************************************************************
      SUBROUTINE ESPAVF( ICOMP, INARR, NNDS, IWORK, MHT, MHL, MHM, 
     1                   NRAD, NPHI, LH, THE, VEC, XARR, RVALS,
     2                   PVALS, WORK1, WORK2, COEFM, VMISS, ESPAV )
      IMPLICIT NONE
C____________________________________________________________________C
C Variable declarations - Parameters ................................C
      INTEGER ICOMP, INARR( * ), NNDS, IWORK( NNDS ), MHT( * ),
     1        MHL( * ), MHM( * ), NRAD, NPHI, LH
      DOUBLE PRECISION THE, VEC( * ), XARR( * ), RVALS( NRAD ),
     1                 PVALS( NPHI ), WORK1( NNDS ),
     2                 WORK2( NNDS ), COEFM( NNDS, NNDS ),
     3                 ESPAV( NRAD, NPHI ), VMISS
C____________________________________________________________________C
C Variable declarations - Working variables .........................C
      INTEGER IRAD, IPHI, NR, NH, ITYPE, IPOL, ITOR, L, M, IH
...
C
      ...
      DO IRAD = 1, NRAD
        ...
        IF ( RAD.LT.RI .OR. RAD.GT.RO .OR. RAD.LT.DLOW ) THEN
          DO IPHI = 1, NPHI
            ESPAV( IRAD, IPHI ) = VMISS
          ENDDO
        ELSE
          DO IPHI = 1, NPHI
            ESPAV( IRAD, IPHI ) = 0.0d0
          ENDDO
C         .
            ...
            IF ( ICOMP.EQ.7 ) THEN
              ...
              DO IPHI = 1, NPHI
                ...
                ELSE
                  PHTRM  = DCOS( M*PHI )
                ENDIF
                ESPAV( IRAD, IPHI ) = ESPAV( IRAD, IPHI ) +
     1                                 WORK1( 1 )*P*PHTRM
              ENDDO
              GOTO 50
            ENDIF
