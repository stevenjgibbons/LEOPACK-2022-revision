C*********************************************************************
C subroutine Kumar Roberts Velocity HarMonic Find ********************
C            -     -       -        -        -    ********************
C Steve Gibbons Thu Oct  7 08:34:21 BST 1999                         C
C____________________________________________________________________C
C                                                                    C
C Enters the indices to be able to store the Kumar Roberts velocity  C
C harmonics into the arrays MHT, MHL, MHM. For a supplied number     C
C IHARM; KRVHMF will fill the following array elements ...           C
C                                                                    C
C  MHT( iharm    ) = 2, MHL( iharm    ) = 1 and MHM( iharm    ) = 0  C
C  MHT( iharm + 1) = 1, MHL( iharm + 1) = 2 and MHM( iharm + 1) = 0  C
C  MHT( iharm + 2) = 1, MHL( iharm + 2) = 2 and MHM( iharm + 2) = -2 C
C  MHT( iharm + 3) = 1, MHL( iharm + 3) = 2 and MHM( iharm + 3) = 2  C
C                                                                    C
C  It is then checked that NH = INARR( 3 ) is atleast IHARM + 3.     C
C                                                                    C
C____________________________________________________________________C
C                                                                    C
C Input variables :-                                                 C
...
C                MHM( ih ) = -m                                      C
C                                                                    C
C     IHARM     : Harmonic at which KR velocity must begin.          C
C                                                                    C
C                                                                    C
C____________________________________________________________________C
C
C*********************************************************************
      SUBROUTINE KRVHMF( INARR, MHT, MHL, MHM, IHARM )
      IMPLICIT NONE
C____________________________________________________________________C
...
C
      NH  = INARR( 3 )
C     .
      IF ( NH.LT.(IHARM+3) ) THEN
        PRINT *,' Subroutine KRVHMF.'
