C*********************************************************************
C subroutine Transform Semi-Implicit Dynamo Time Step routine        *
C            -         -    -        -      -    -                   *
...
C MCVDMF. TSIDTS will advance the solution by one time step (size    C
C DELTAT).                                                           C
C____________________________________________________________________C
C                                                                    C
...
C
C*********************************************************************
      SUBROUTINE TSIDTS( KL, NBN, NCFM, NR, NDRVM, NH, NDCS, MHT, MHL,
     1             MHM, MHP, MHTR, LH, MMAX, M0, NPHP, NTHP, VEC, 
     2             SVFDC, FDCM, PA, DPA, GAUX, GAUW, SHC, DSHC, VF1,
     3             VF2, VF3, SF, XARR, N2, NOIT, LULOG, IPIV, MHIBC,
     3             MHOBC, DMAT, DPARS, V0, V1, V2, V3, V4,
     4             DTV, RFI, RFI1, VEC1, RQST1, RQST2, RQST3, RQSTA )
      IMPLICIT NONE
C____________________________________________________________________C
...
C Variable declarations - Working variables .........................C
      INTEGER ITMX, ILN, IRN, IHD, NDRVS, INARR( 3 ), INC,
     1        INFO, NBAND, NRHS
      DOUBLE PRECISION DELTAT, TOL, CFAC, CA, CB1, CB2, CC, CD, CE,
     1                 CF, CG, CH, CI, CJ, CK, CL, CM, AOLD,
     2                 AVEC1, DNRM2, FAC, DIFF, ODIFF
      CHARACTER *(1) TRANS
      PARAMETER ( INC = 1, TRANS = 'N', NRHS = 1 )
C____________________________________________________________________C
...
C
      CALL DVECZ( RFI, N2 )
      CALL STSVFT( NR, INARR, MHT, MHL, MHM, MHTR, LH, NCFM,
...
C
 ...
 81   FORMAT('tsidts: Predictor norm = ',1PD16.7)
C
      ITMX = NOIT
      NOIT = 0
 50   CONTINUE
 ...
 83     FORMAT('tsidts: Max. iterations exceeded.')
        NOIT = -1
        RETURN
      ENDIF
C
...
C
 ...
 84   FORMAT('tsidts: Iteration ',I4,' Solution norm = ',1PD16.7)
