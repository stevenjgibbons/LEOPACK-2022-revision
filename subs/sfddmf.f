C*********************************************************************
C subroutine Stress Free Dynamo Diffusion Matrix Form ****************
C            -      -    -      -         -      -    ****************
C Steve Gibbons Fri Mar  3 14:33:35 GMT 2000                         C
...
C                                                                    C
C For harmonics, ih, which satisfy MHT( ih ) = 2, MHP( ih ) = is     C
C and MHBCS( is ) = 6, MHL( ih ) = 1 - the row corresponding to      C
C radial grid node NR/2 is zeroed and 1.0d0 put in its diagonal      C
C element. This allows stress free velocities to be time-stepped     C
C provided that a suitable treatment of these functions is           C
C implemented (by using a version of the Woodbury formula for ex.)   C
C when the system is solved.                                         C
C                                                                    C
C____________________________________________________________________C
C                                                                    C
C Input variables :-                                                 C
...
C                 This is the number of harmonics, ih, which satisfy C
C                 MHT( ih ) = 2, MHP( ih ) = is and MHBCS( is ) = 6, C
C                 MHL( ih ) = 1. NTS can take values 0, 1 or 3.      C
C                                                                    C
...
C Variable declarations - Working variables .........................C
     ...
     1        ILNT, IRNT, ICMPO, IS, IRC, IND, INDFUN, IRAD, IH
...
C
     ...
     3           IMF, KL, KL, KL, NDCS )
C
C Find out the rows which correspond to node NR/2
C of harmonics, ih, which satsify conditions of
C MHT( ih ) = 2 ( --> tor. vel.),
C MHL( ih ) = 1
C MHBCS( MHP( ih ) ) = 6
C Then zero each of these rows and put 1.0d0
...
C
      ...
      DO IH = 1, NH
        IS = MHP( IH )
C       .
        IF ( MHBCS( IS ).EQ.1 ) THEN
          PRINT *,' Subroutine SFDDMF.'
          PRINT *,' Harmonic ',IH,' has no homog.'
          PRINT *,' boundary specification.'
          PRINT *,' Program aborted.'
          STOP
        ENDIF
C       .
        IF (     MHBCS( IS ).EQ.6 .AND. MHT( IH ).EQ.2
     1           .AND. MHL( IH ).EQ.1  ) THEN
          NTS = NTS + 1
