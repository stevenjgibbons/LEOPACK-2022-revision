C*********************************************************************
C subroutine Arbitrary Node Velocity dot Gradient of Theta Terms *****
C            -         -    -            -           -     -     *****
C Steve Gibbons Mon Nov 22 08:00:19 GMT 1999                         C
C____________________________________________________________________C
C                                                                    C
C Provides, via ANNLCA, the matrix terms for the v.Grad(theta)       C
...
C
C*********************************************************************
      SUBROUTINE ANVGTT( CVEC, RAD, IPARS, DPARS, IHD, IH0, IHD0, 
...
C
      ...
      DOUBLE PRECISION FAC, RAD2, LOW, FLA, FLB, SQRLL1
...
C
      RAD2 = RAD*RAD
C
C Take derivative of harmonic IH0 at r = RAD
C
...
C
     ...
     1     IHD.EQ.1 .AND. IHD0.EQ.0 ) THEN
        CVEC( 2 ) = FLA*FLA*FAC*WORK1( 1 )/RAD
        RETURN
      ENDIF
C
...
C
     ...
     1     IHD.EQ.0 .AND. IHD0.EQ.1 ) THEN
        CVEC( 1 ) = FLA*FLA*FAC*WORK1( 2 )/RAD
        RETURN
      ENDIF
C
...
C
     ...
     1     IHD.EQ.0 .AND. IHD0.EQ.1 ) THEN
        CVEC( 1 ) = FLA*FLB*FAC*( WORK1(2)/RAD + WORK1(1)/RAD2 )
        RETURN
      ENDIF
C
...
C
     ...
     1     IHD.EQ.1 .AND. IHD0.EQ.0 ) THEN
        CVEC( 1 ) = FLA*FLB*FAC*WORK1(1)/RAD2
        CVEC( 2 ) = FLA*FLB*FAC*WORK1(1)/RAD
        RETURN
      ENDIF
C
...
C
     ...
     1     IHD.EQ.0 .AND. IHD0.EQ.0 ) THEN
        CVEC( 1 ) = (-1.0d0)*FLA*FLB*FAC*WORK1(1)/RAD
        RETURN
      ENDIF
C
