C*********************************************************************
C subroutine TeMPerature Matrix Boundary condtion Enforce ************
C            - --        -      -                 -       ************
C Steve Gibbons Tue Oct 19 10:15:19 BST 1999                         C
C____________________________________________________________________C
C                                                                    C
C Applies a boundary condition to all the temperature                C
...
C either HTMRBE (homogeneous temperature boundaries) or              C
C ITMRBE (inhomogeneous temperature boundaries).                     C
C                                                                    C
...
C  ---------                                                         C
C                                                                    C
C     BCTHE     : Boundary condition flag for temperature            C
C                 'HF' or 'hf' for fixed heat flux.                  C
C                 'TM' or 'tm' for fixed temperature.                C
C     CHBND     : Boundary flag (*). Either 'Inner' or 'Outer'       C
C                                                                    C
C____________________________________________________________________C
C
C*********************************************************************
      SUBROUTINE TMPMBE( N1, N2, KL, KU, KLE, IMF, NR, INARR, MHTI,
...
C     .
      ...
      ENDIF
C     .
C     . Check the validity of BCTHE
C     .
      IF ( BCTHE.EQ.'TM' .OR. BCTHE.EQ.'tm' ) THEN
        FIXEDH = .FALSE.
        GOTO 40
      ENDIF
C     .
      IF ( BCTHE.EQ.'HF' .OR. BCTHE.EQ.'hf' ) THEN
        FIXEDH = .TRUE.
        GOTO 40
      ENDIF
C     .
...
C     .
      ...
      ENDIF
C     .
      PRINT *,' Subroutine TMPMBE. CHBND = ', CHBND
      PRINT *,' Program aborted.'
      STOP
C     .
 50   CONTINUE
