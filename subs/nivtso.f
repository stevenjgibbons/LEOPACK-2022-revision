...
C SFDDMF. NIVTSO will advance the solution by one time step (size    C
C DELTAT).                                                           C
...
C
C*********************************************************************
      SUBROUTINE NIVTSO( KL, NBN, NCFM, NR, NDRVM, NH, NDCS, MHT, MHL,
...
C
 ...
 81   FORMAT('nivtso: Predictor norm = ',1PD16.7)
C
      ITMX = NOIT
      NOIT = 0
 50   CONTINUE
 ...
 83     FORMAT('nivtso: Max. iterations exceeded.')
        NOIT = -1
        RETURN
      ENDIF
C
...
C
 ...
 84   FORMAT('nivtso: Iteration ',I4,' Solution norm = ',1PD16.7)
