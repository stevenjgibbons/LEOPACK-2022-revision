...
C                                         -                    *******
C Steve Gibbons Mon Dec 13 11:27:42 GMT 1999                         C
...
C                                                                    C
C Differs from SICRBR in that the non-linear terms are calculated    C
...
C
C*********************************************************************
      SUBROUTINE SICRBT( NR,INARR,MHT,MHL,MHM,MHP,MHTR,NBN,KL,NCFM,
     1   NDRVM,N1A,N1B,N2,NDCS,NTHP,NPHP,MMAX,LH,IEV,MHIBC,MHOBC,NEV,
     2   NCV,NCVM,MXIT,IPIV,LULOG,SVFDC,A,B,XARR,GAUX,GAUW,PA,DPA,
     3   SBRVEC,RESID,W2,WVEC,F1,F2,F3,VF1,VF2,VF3,QST,SF,SHC,SELECT,
     4   ARTOL,DRSV,PARAM,MXATT,IERR,DR,DI,D3,WORKEV,WORKD,WORKL,V,
     5   IN0,MT0,ML0,MM0,MP0,VEC0 )
      IMPLICIT NONE
C____________________________________________________________________C
...
C
      ...
      ENDIF
C
C Set up matrix B with Reynolds number dependent terms
C
      ICLS = 1
      PARAM(  1 ) = ZERO
      PARAM(  2 ) = ZERO
...
C
      ...
      ENDIF
C
C Copy Rey dependent terms into matrix.
C
      FACA = ZERO
      FACB = REY
