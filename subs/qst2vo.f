C*********************************************************************
C subroutine QST to Vector function (Optimised) **********************
C            --- -- -                -          **********************
C Steve Gibbons Mon Oct  8 15:15:44 WEST 2001                        C
...
C
C*********************************************************************
      SUBROUTINE QST2VO( QST, VF, GAUX, PA, DPA, FTF1, FTF2, FTF3,
     1                    LH, NTHPTS, NPHPTS, MMAX, ZCOEF, M0 )
      IMPLICIT NONE
C____________________________________________________________________C
...
C Variable declarations - Working variables .........................C
      INTEGER          IOP, ISIGN, ITHREE, ITHETA, LENV, IPHI, MM,
     1                 IP, IND1, IND2, L, M, INDFTC, INDFTS, IQST
      DOUBLE PRECISION ZERO, X, SINE, TERM1, TERM2, FACL
...
C
c     IF ( NTHPTS.LE.LH ) THEN
c        PRINT *,' Subroutine QST2VO.'
c        PRINT *,' NTHPTS must be atleast ', LH + 1
c        PRINT *,' Program stopped.'
c        STOP
c     ENDIF
C
c     IF ( NPHPTS.LE.(2*MMAX+1) ) THEN
c        PRINT *,' Subroutine QST2VO.'
c        PRINT *,' NPHPTS must be atleast ',(2*MMAX+1)
c        PRINT *,' Program stopped.'
c        STOP
c     ENDIF
C
...
C ............. start to loop around Harmonics ......................
         DO L = 1, LH
            ...
            FACL   = SQRLL1(L)
            FTF1( 1 ) = FTF1( INDFTC ) 
     1                   + PA( IP, ITHETA )*QST( IQST, 1 )
            FTF2( 1 ) = FTF2( INDFTC ) 
     1                   + DPA( IP, ITHETA )*QST( IQST, 2 )/FACL
            FTF3( 1 ) = FTF3( INDFTC ) 
     1                   + DPA( IP, ITHETA )*QST( IQST, 3 )/FACL
         ENDDO
C ................. end of case M = 0
         DO MM = 1, MIN( LH, MMAX )/M0
            M      = MM*M0
            INDFTC = 2*MM + 1
            INDFTS = INDFTC + 1
            DO L = M, LH
               ...
               IND2  = L*L + 2*M
               IND1  = IND2 - 1
               TERM1 = M*PA( IP, ITHETA )/( SINE*FACL )
               TERM2 = DPA( IP, ITHETA )/FACL
C
...
C ...................................................................
         DO IPHI = 1, NPHPTS
            VF ( IPHI , ITHETA, 1 ) = FTF1( 2*IPHI - 1 ) + ZCOEF
