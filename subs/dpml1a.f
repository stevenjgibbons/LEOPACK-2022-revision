C*********************************************************************
C subroutine Double Precision Matrix Laplacian 1 (poloidal) Add ******
C            -      -         -      -         -            -   ******
C Steve Gibbons 16.3.99                                              C
C____________________________________________________________________C
C Adds the term to the double precision matrix resulting from        C
C the curl of the laplacian of the poloidal velocity.                C
C This results in a toroidal harmonic with a -D_l^2 operator.        C
C____________________________________________________________________C
C Input variables :-                                                 C
C ===============                                                    C
C  Integer                                                           C
...
C
C*********************************************************************
      SUBROUTINE DPML1A ( N1, N2, NR, NH, KL, KU, KLE, IMF, MHT,
...
C
      IF ( N2.NE.NH*NR ) THEN
        PRINT *,' Subroutine DPML1A, bad array size'
...
C the equivalent toroidal harmonics
C
      DO IIH = 1, NH
...
c       . toroidal harmonic.
c       . However, note that for numerical convenience,
c       . the toroidal flow equations are swapped with
c       . the poloidal flow equations and so we simply
c       . put the result in the same harmonic
c
         INTPAR( 1 ) = MHL( IIH )
         INTPAR( 2 ) = 2
