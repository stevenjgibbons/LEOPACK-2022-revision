...
C            -      -               -            -        -         **
C Steve Gibbons Fri Jul 14 11:06:16 BST 2000          :Optimised.    C
C____________________________________________________________________C
C                                                                    C
C If the vector VI contains radial functions to NHI spherical harm.s C
C with type defined by MHTI, MHLI and MHMI, then RVVGTO will add FAC C
...
C                                                                    C
C Important! The velocity, v, is supplied in the array RVFO, in real C
C space, and must be obtained in advance by calling RQSRVO.          C
C RVVGTO does NOT extract the velocity from VI. This is done         C
C entirely to reduce the programming load and ease of checking.      C
C (Actually done for lots of good reasons now!)                      C
C                                                                    C
C Grad theta IS calculated by RVVGTO from the harmonics in VI with   C
C MHTI( ih ) = 3.                                                    C
C                                                                    C
C____________________________________________________________________C
C                                                                    C
...
C                                                                    C
C     RVFO      : Velocity. dim. ( 3, NPHPTS, NTHPTS, NR )           C
C     VFO       : Work arr. dim. ( 3, NPHPTS, NTHPTS )               C
...
C
C*********************************************************************
      SUBROUTINE RVVGTO ( NR, LH, MMAX, MHTI, MHLI, MHMI, V0, V1,
     1             INARRI, MHTO, MHLO, MHMO, VO, INARRO, RVFO, VFO,
...
C       . supplied to GRINVT - this will fill VFO
C       . with Grad( theta )
C       .
        CALL GRINVO( SHC, DSHC, GAUX, RAD, PA, DPA, FTF1, FTF2,
     1               FTF3, VFO, LH, NTHPTS, NPHPTS, MMAX, DZCOEF )
C       .
C       . RVF now contains V and VF now contains Grad( theta )
C       . so now take scalar product.
C       .
        CALL RVFDVO( RVFO, VFO, SF, IR, NR, NPHPTS, NTHPTS )
C       .
