C*********************************************************************
C subroutine ARRAY POLAR ARROWS 2 ************************************
C            ----- ----- ------ - ************************************
C Steve Gibbons Wed Jul  4 18:00:56 WEST 2001                        C
...
C
C*********************************************************************
      SUBROUTINE ARRAY_POLAR_ARROWS2( VC1, VC2, NPBA, RLONG, RHEAD,
     1                               IW, SCALF )
      IMPLICIT NONE
C____________________________________________________________________C
...
C
     ...
     1         ABS( VMAG ).LT.RLIM    ) MGMAX = ABS( VMAG )
          IF ( ABS( SVAL ).LT.RLIM .AND.
     1         SVAL.GT.SFMAX ) SFMAX = SVAL
          IF ( ABS( SVAL ).LT.RLIM .AND.
     1         SVAL.LT.SFMIN ) SFMIN = SVAL
        ENDDO
      ENDDO
C
      SFHALF = 0.5*(SFMIN+SFMAX)
C
...
C         .
          IF ( SVAL.LE.SFHALF ) CALL PGSCI( 1 )
          IF ( SVAL.GT.SFHALF ) CALL PGSCI( 0 )
          CALL PGSCH( RMAG )
          CALL PGSLW( IW )
